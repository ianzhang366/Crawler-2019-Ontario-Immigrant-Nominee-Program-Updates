...
def format_post(info, content, email_source):
	logger.info('ENTRY')
...
def is_new_email(info, json_file, email_source):
	...
	if need_to_send:
		...
		if time_check.hour == 17:
			...
			d_msg = format_post(info, content, email_source)
			if (d_short_msg in past_posts.keys()) == False:
				past_posts[d_short_msg] = d_short_msg
				_send_email(d_msg, title= config.EMIL_CONTENT.END_OF_DAY_TITLE)
				flag = 1
		if time_check.hour == 8:
			...
			if (d_short_msg in past_posts.keys()) == False:
				past_posts[d_short_msg] = d_short_msg
				_send_email(d_msg, title= config.EMIL_CONTENT.BEGIN_OF_DAY_TITLE)
				flag = 1
		logger.debug('is_new_email() check hour:%s, if sent an email %s',time_check, flag)
		logger.info('EXIT')
	logger.info('EXIT')
...
def _main():
	...
	logger.debug('Main: email_handler_main() Run time is EST:%s', time_check.strftime(config.LOG_CONFIG.datefmt))
	logger.info('ENTEY')
...
# 	print info
	if info:
		if len(info):
			logger.debug('email_handler_main() Posts: '+ ' '.join(info[0]['content']))
		else:
			logger.debug('email_handler_main() Posts: []')
	else:
		logger.debug('email_handler_main() Posts: is NONE')
		logger.debug('email_handler_main() JSON Path: '+json_file)
	email_source = platform.uname()[1]
	is_new_email(info, json_file, email_source)
	if info:
		logger.debug('email_handler_main() Posts: %s', info[-20:])
		logger.info('EXIT')
	else:
		logger.debug('email_handler_main() Posts: %s', info)
		logger.info('EXIT')
